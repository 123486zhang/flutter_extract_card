...
class RegisterPageState extends State<RegisterPage> {
      ...
      query.queryObjects().then((data) {
        ...
        if (users == null || users.length == 0) {
          ...
          user.register().then((value) {
            if (value != null) {
              showToast(JYLocalizations.localizedString("注册成功"));
              ...
              showToast(JYLocalizations.localizedString("注册失败"));
            }
          });
        } else {
          showToast(JYLocalizations.localizedString("用户名已存在"));
        }
      }).catchError((e) {
        showToast('网络错误');
      });
  ...
  bool checkInfo() {
    if (usernameController.text.length < 6) {
      showToast(JYLocalizations.localizedString("用户名长度必须大于6"));
      return false;
    }
    if (passwordController.text.length < 6) {
      showToast(JYLocalizations.localizedString("密码长度必须大于5"));
      return false;
    }
    return true;
  }
